...
def dpo_hf_collate_fn(data):
    ...
    return {"data": (input_ids_y1, input_ids_y2,
...
class DPOHFDataset(Dataset):
    def __init__(self, get_hdf5,
                 data_in_memory=True):
        self.data_in_memory = data_in_memory
        self.get_hdf5 = get_hdf5

        dpo_hdf5 = self.get_hdf5()
        self.num_examples_per_prompt = dpo_hdf5.attrs['num_examples_per_prompt']
        self.num_pairs_per_prompt = (self.num_examples_per_prompt
                                     * (self.num_examples_per_prompt - 1) // 2)
        self.num_prompts = dpo_hdf5.attrs['num_prompts']

        assert (len(dpo_hdf5['input_ids'])
    ...
    def open_hdf5(self):
        self.dpo_hdf5 = self.get_hdf5()
        self.input_ids = self.dpo_hdf5['input_ids']
        self.attention_mask = self.dpo_hdf5['attention_mask']
        self.logp_masks = self.dpo_hdf5['logp_masks']
        self.logps_y = self.dpo_hdf5['logps_y']
        self.energies = self.dpo_hdf5['energies']
    ...
    def __getitem__(self, idx):
        ...
        return (torch.tensor(self.input_ids[idx_1]), torch.tensor(self.input_ids[idx_2]),
                torch.tensor(self.attention_mask[idx_1]), torch.tensor(
                    self.attention_mask[idx_2]),
                torch.tensor(self.logp_masks[idx_1]), torch.tensor(
                    self.logp_masks[idx_2]),
                torch.tensor(self.logps_y[idx_1]), torch.tensor(
                    self.logps_y[idx_2]),
